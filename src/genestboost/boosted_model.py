...
class BoostedModel:
    ...
    def __init__(
        ...
        self.model_callback_kwargs = dict() if model_callback_kwargs is None else model_callback_kwargs
        ...
        self.activation_callback = (lambda x: x) if activation_callback is None else activation_callback
    ...
    def _split_data(self, X, yt):
        ...
        if self.validation_stratify == True:
            ...
            for group in np.unique(yt):
                mask = (yt == group)
                n = np.sum(mask)
                index = np.random.choice(np.arange(yt.shape[0])[mask],
            ...
            self._vindex = np.random.choice(yt.shape[0],
                                            int(self.validation_fraction * yt.shape[0]),
                                            replace=False)
    ...
    def _compute_beta(self, yt, eta_p, next_model_preds):
    ...
    def _track_loss(self, yt_train, yp_train, weights_train, yt_val, yp_val, weights_val):
        if self._loss_list is not None:
            tloss = np.sum(self._loss(yt_train, yp_train) * weights_train / np.sum(weights_train))
            if yt_val is None or yp_val is None:
                vloss = None
            else:
                vloss = np.sum(self._loss(yt_val, yp_val) * weights_val) / np.sum(weights_val)
            self._loss_list.append((tloss, vloss))

    def _line_search_constant(self, yt, eta_p, next_model_preds):
        return 1.0
    ...
    def _line_search_shrinking(self, yt, eta_p, next_model_preds):
    ...
    def _line_search_decaying(self, yt, eta_p, next_model_preds):
    ...
    def fit(self,
        ...
        yp_train, eta_p_train = self.initialize_model(X_train,
                                                      yt_train,
                                                      weights_train)
        ...
        if X_val is not None:
            ...
            yp_val = None
        self._track_loss(yt_train, yp_train, weights_train,
                         yt_val, yp_val, weights_val)

        # perform boosting iterations
        for _ in range(iterations):
            yp_train, eta_p_train = self.boost(X_train,
                                               yt_train,
                                               yp_train,
                                               eta_p_train,
                                               self.model_callback,
    ...
    def decision_function_last_model(self, X: np.ndarray):
    ...
    def prediction_history(self, X: np.ndarray, links=False) -> np.ndarray:
        model_links = self.get_model_links(X)
        lr_array = (
            np
            .hstack([[1], [tup[1] for tup in self._model_list]])
            .reshape((1, -1))
        )
    ...
    def get_model_links(self, X):
        model_links = np.zeros((X.shape[0], len(self._model_list) + 1),
                               dtype=np.float64)
        eta_p = self._model_init.predict(X)
        model_links[:, 0] = eta_p
    ...
    class InitialModel:
        def __init__(self, link, init_type=None):
            self._link = link
            self._init_type = init_type
            self._value = None

        def fit(self, X, yt, weights=None):
            weights = np.ones_like(yt) if weights is None else weights
